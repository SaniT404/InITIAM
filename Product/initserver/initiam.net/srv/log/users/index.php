...
$stmt = $dbh->prepare("INSERT INTO `UserInformation` VALUES (?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
foreach($data_expanded as $d){
	...
	if(is_array($tmplode) && $tmplode[1] != "NULL"){
		...
		$stmt->execute(array($clientid,	//		 ClientID
						...
						$tmp[3],		// [3] = LastUseTime
						$tmp[4],		// [4] = Status
						$tmp[5],		// [5] = HealthStatus
						$tmp[6],		// [6] = DesktopSize
						$tmp[7],		// [7] = DocumentsSize
						$tmp[8],		// [8] = PicturesSize
						$tmp[9],		// [9] = MusicSize
						$tmp[10],		// [10] = VideosSize
						$tmp[11],		// [11] = FavoritesSize
						$tmp[12],		// [12] = DownloadsSize
						$tmp[13],		// [13] = LinksSize
						$tmp[14],		// [14] = SearchesSize
						$tmp[15],		// [15] = SavedGamesSize
						$tmp[16]));		// [22] = AppdataRoamingSize
	}
}
