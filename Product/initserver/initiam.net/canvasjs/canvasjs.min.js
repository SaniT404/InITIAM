...
width:c.width<c.maxWidth?c.width:c.maxWidth})}}}function e(){var a,b,c,d,e,f=[];e=!1;c=0;for(var g,k=N-Z-U/2,k=t.options.indexLabelMaxWidth?t.indexLabelMaxWidth>k?k:t.indexLabelMaxWidth:k,l=I.length-1;0<=l;l--){g=x[I[l].id];c=I[l];d=c.textBlock;b=(a=q(l)<P.length?I[q(l)]:null)?a.textBlock:null;c=c.height;a&&d.y+c+da>b.y&&(e=!0);c=g.indexLabelMaxWidth||k;if(c>k||0>c)c=k;f.push(c)}if(e)for(l=I.length-1;0<=l;l--)a=P[l],I[l].textBlock.maxWidth=f[f.length-(l+1)],I[l].textBlock.measureText(),I[l].textBlock.x=
...
(k&&e+d+da>+l.y.toFixed(6))&&(d=g.y+d+da-l.y,e=a(r,-d),e<d&&(0<e&&(d-=e),e=a(q(r),d),e!=d)));r--);b()}}function h(a,b){return(b<P.length?I[b].textBlock.y:t.reversed?S+C:qa+C)-(0>a?t.reversed?qa-C:S-C:I[a].textBlock.y+I[a].height+da)}function m(a,b,c){var d,e,g,l=[],h=C,p=[];-1!==b&&(0<=X.indexOf(b)?(e=X.indexOf(b),X.splice(e,1)):(X.push(b),X=X.sort(function(a,b){return a-b})));if(0===X.length)l=ga;else{e=C*(1!=X.length||0!=X[0]&&X[0]!=P.length-1?2:1)/k();for(var q=0;q<P.length;q++){if(1==X.length&&
0==X[0]){if(0===q){l.push(ga[q]);d=h;continue}}else 0===q&&(d=-1*h);l.push(ga[q]+d);if(0<=X.indexOf(q)||q<P.length&&0<=X.indexOf(q+1))d+=e}}g=function(){for(var a=[],b=0;b<P.length;b++)a.push(l[b]-P[b].y1);return a}();var u={startTime:(new Date).getTime(),duration:c||500,easingFunction:function(a,b,c,d){return L.easing.easeOutQuart(a,b,c,d)},changeSection:function(a){for(var b,c,d=0;d<P.length;d++)b=g[d],c=P[d],b*=a,"undefined"===typeof p[d]&&(p[d]=0),0>p&&(p*=-1),c.y1+=b-p[d],c.y2+=b-p[d],c.y3+=
...
0.35*E:t.neckHeight,G=s(t.options.neckWidth)?0.25*a:t.neckWidth,"string"===typeof ba&&ba.match(/%$/)?(ba=parseInt(ba),ba=ba*E/100):ba=parseInt(ba),"string"===typeof G&&G.match(/%$/)?(G=parseInt(G),G=G*a/100):G=parseInt(G),ba>E?ba=E:0>=ba&&(ba=0),G>a?G=a-0.5:0>=G&&(G=0)):"pyramid"===t.type&&(G=ba=0,t.reversed=t.reversed?!1:!0);var D=H+a/2,V=H,Z=H+a,S=t.reversed?R:Y,M=D-G/2,W=D+G/2,va=t.reversed?Y+ba:R-ba,qa=t.reversed?Y:R;a=[];var D=[],P=[],F=[],T=Y,ca,$=(va-S)/(M-V),ea=-$,H="area"===(t.valueRepresents?
t.valueRepresents:"height")?c():d();if(-1!==H){if(t.reversed)for(F.push(T),G=H.length-1;0<G;G--)T+=H[G],F.push(T);else for(G=0;G<H.length;G++)T+=H[G],F.push(T);if(t.reversed)for(G=0;G<H.length;G++)F[G]<va?(a.push(M),D.push(W),ca=G):(a.push((F[G]-S+$*V)/$),D.push((F[G]-S+ea*Z)/ea));else for(G=0;G<H.length;G++)F[G]<va?(a.push((F[G]-S+$*V)/$),D.push((F[G]-S+ea*Z)/ea),ca=G):(a.push(M),D.push(W));for(G=0;G<H.length-1;G++)T=t.reversed?x[x.length-1-G].color?x[x.length-1-G].color:t._colorSet[(x.length-1-
G)%t._colorSet.length]:x[G].color?x[G].color:t._colorSet[G%t._colorSet.length],G===ca?P.push({x1:a[G],y1:F[G],x2:D[G],y2:F[G],x3:W,y3:va,x4:D[G+1],y4:F[G+1],x5:a[G+1],y5:F[G+1],x6:M,y6:va,id:G,height:F[G+1]-F[G],color:T}):P.push({x1:a[G],y1:F[G],x2:D[G],y2:F[G],x3:D[G+1],y3:F[G+1],x4:a[G+1],y4:F[G+1],id:G,height:F[G+1]-F[G],color:T});var da=2,I=[],ha=!1,X=[],ga=[],H=!1;a=a=0;Aa(X);for(G=0;G<x.length;G++)x[G].exploded&&(H=!0,t.reversed?X.push(x.length-1-G):X.push(G));Q.clearRect(A.x1,A.y1,A.width,
