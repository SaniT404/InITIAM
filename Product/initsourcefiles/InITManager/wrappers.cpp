...
bool installReg()
{
	...
	else
	{
		...
		// Check if guid is still in registry
		std::wstring clientguid;
		HKEY hKey;
		RegOpenKeyEx(HKEY_LOCAL_MACHINE, SERVICE_DEFAULT_REG, NULL, KEY_READ, &hKey);
		GetStringRegKey(hKey, L"clientguid", clientguid, L"null");
		RegCloseKey(hKey);
		// If guid not already assigned to machine:
		if (clientguid == L"null")
		{
			...
			// Successfully created guid - set clientguid
			else
			{
...
bool installDir()
{
	...
	{
		std::wcout << "InIT directory successfully created" << std::endl;
		// Service sub-directory
		if (CreateDirectory(SERVICE_HOME_PATH, NULL) || ERROR_ALREADY_EXISTS == GetLastError())
		{
			std::wcout << "InIT/InITManager directory successfully created" << std::endl;
			// Service's log sub-directory
			if (CreateDirectory(SERVICE_LOG_PATH, NULL) || ERROR_ALREADY_EXISTS == GetLastError())
			{
				std::wcout << "InIT/InITManager/logs directory successfully created" << std::endl;
			}
			else
			{
				std::wcout << "Failed to create InIT/InITManager/logs directory." << std::endl;
				return false;
			}
		}
		else
		{
			std::wcout << "Failed to create InIT/InITManager directory." << std::endl;
			return false;
		}
	}
	else
	{
		std::wcout << "Failed to create InIT directory." << std::endl;
		return false;
	}
	return true;
}
